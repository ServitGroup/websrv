...
class App_optionController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'app_options')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        $qry = App_option::query();
        $qry->whereRaw('1 = 1');
        $vkw = '';
        if ($kws) {
        ...
        $info = Dbinfo::where('table_name', 'app_options')->first();
        //---addition----
        $method = [];
        $domains = [];
