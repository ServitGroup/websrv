...
class Omise_recipientController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'omise_recipients')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        $qry = Omise_recipient::query();
        $qry->whereRaw('1 = 1');
        $vkw = '';
        if ($kws) {
        ...
        $info = Dbinfo::where('table_name', 'omise_recipients')->first();
        //---addition----
        $method = [];
        $domains = [];
