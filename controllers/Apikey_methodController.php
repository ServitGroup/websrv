...
class Apikey_methodController extends BaseController {
...
public function all($page = 1, $perpage = 10, $kw = '', $ajax = 0){
        ...
        $columns = Column::where('table_id', 'apikey_method')->orderBy('sort', 'asc')->get();
        $kws = [];
        if ($kw) {
            $kws = explode(',', $kw);
        }
        $qry = Apikey_method::query();
        $qry->whereRaw('1 = 1');
        $vkw = '';
        if ($kws) {
        ...
        $info = Dbinfo::where('table_name', 'apikey_method')->first();
        //---addition----
        $method = [];
        $domains = [];
